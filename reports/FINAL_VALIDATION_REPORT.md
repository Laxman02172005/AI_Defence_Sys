...
## 7. Round 1 vs Round 2 — artifact is STALE relative to the brief

The brief states ATO Round 2 recall regressed to **96.08%**. The on-disk
`round1_vs_round2_report.json` (both the `cascade_comparison` and
`single_stage_comparison` sections) instead shows ATO Round 2 recall of
**97.98%** (`0.9797979797979798`), with Round 1 at 98.97% in both cases.
Neither section reproduces the exact 96.08% figure the brief cites. This
means the artifact currently on disk was generated by a different run
than the one the brief's numbers describe — evidence of exactly the kind
of artifact staleness the brief asks to be surfaced honestly, not
smoothed over. I have **not** attempted to explain or fix this
