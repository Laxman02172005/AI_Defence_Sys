...
def expected_cost(
    ...
    c = np.zeros(len(y))
    # ALLOW: legit costs 0; fraud costs its full $ exposure (no recovery assumed)
...
# ---------------------------------------------------------------------------
def diagnose_prevalence_bug(y: np.ndarray, proba: np.ndarray, dollars: np.ndarray, cost: CostModel):
    naive_cost = CostModel(**{**asdict(cost), "assumed_production_fraud_rate": None})
    naive = optimize_thresholds(y, proba, dollars, naive_cost)
    corrected = optimize_thresholds(y, proba, dollars, cost)
...
def main():
    ...
    naive, corrected = diagnose_prevalence_bug(y, proba, dollars, cost)

    print("=" * 72)
    print("FINAL POLICY (prevalence-corrected, scored on Risk Fusion output)")
    print("=" * 72)
    ...
    with open(out_path, "w") as f:
        json.dump({"naive": naive, "corrected": corrected, "score_source": score_source}, f, indent=2)
    print(f"\nSaved to {out_path}")
